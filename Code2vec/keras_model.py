...
class Code2VecModel(Code2VecModelBase):
    ...
    def _save_inner_model(self, path):
        if self.config.RELEASE:
            ...
            print('in h5')
            self.keras_train_model.save(path + '__entire-model' + '/tada.h5')
    ...
    def _load_inner_model(self):
        self._create_keras_model()
        self._compile_keras_model()
        path = '/Users/Vesal/Desktop/code2vec-master 2/models/new_keras_models__entire-model/udemy.h5'
        # path = '/Users/Vesal/Desktop/DeepMutation++/rnn_mutation/src/lstm-mutants/WPR_input_0.05_0_0.1_2.h5'
        # model_new = load_model(path , custom_objects={'AttentionLayer' : AttentionLayer})
        # # model_new = load_model(path + '__entire-model' + '/mo.h5', custom_objects={'AttentionLayer' : AttentionLayer})
        # self.keras_train_model = model_new
        # print(self.config.entire_model_load_path)
        self.keras_train_model.load_weights(path)
        # self.keras_train_model.load_weights(self.config.entire_model_load_path + '/mo.h5')
